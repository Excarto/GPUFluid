...
__kernel
void advect4RK2D(__global const float* vxdat, __global const float* vydat,
		  ...
		  __global const float* indat4, __global float* outdat4){
	...
	for (uint i = 0; i < CELLS_PER_ITEM; i++){
		...
		outdat4[datIndex] = bilinInterp(indat4, &bilin);
