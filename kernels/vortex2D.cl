...
void vortex2D(__global const float* invelX,
	...
	cellX = blockCellX + cellX + 1;
	cellY = blockCellY + cellY + 1;
	
	if (cellX <= BORDER_SIZE || cellX >= SIZE_X-BORDER_SIZE-1)
		return;
	if (cellY <= BORDER_SIZE || cellY >= SIZE_Y-BORDER_SIZE-1)
		return;
	
	uint index = cellY*SIZE_X + cellX;
